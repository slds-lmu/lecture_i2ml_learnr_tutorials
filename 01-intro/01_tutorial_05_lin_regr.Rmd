...
- Get to know how to fit a simple non-linear regression model in `mlr`

### Preparation

1.  *(T)* Watch the following video  (sorry, rather low volume...):
    <center>
    ![](https://youtu.be/saf0rLmoEjE){width="75%"}
    </center>
    <a href="https://github.com/compstat-lmu/lecture_i2ml/blob/master/slides-pdf/slides-limo.pdf" target="_blank">Slideset</a>
...
```{r mtcars-task, exercise=TRUE, exercise.lines=5, exercise.checker=taskChecker("mtcars_task")}
task_data =
mtcars_task =
```
...
```{r lm-train, exercise=TRUE, exericse.lines=5, exercise.checker=modelChecker("lm_model")}
lm_model =
```
