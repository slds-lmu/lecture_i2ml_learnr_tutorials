---
title: "Introduction to Machine Learning - Supervised Regression"
output:
  learnr::tutorial:
    theme: cosmo
    css: css/custom.css
...
modelChecker = function (model_name)
{
  ...
  fun = function (label, user_code, check_code, envir_result, evaluate_result, ...)
  {
    ...
    model_hash = digest::digest(c(
      sort(lm_learner$state$train_task$feature_names),
...
```{r, child="tutorial-supervised-regression-losses.Rmd"}
```

```{r, child="tutorial-supervised-regression-linearmodel.Rmd"}
```

```{r, child="tutorial-supervised-regression-polynomials.Rmd"}
```

```{r, child="tutorial-supervised-regression-knn.Rmd"}
```
