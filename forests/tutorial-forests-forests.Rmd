...
Define the learner with `predict_type = "prob"` and `num.trees = 1000`. We are using the `classif.ranger`. Visualize the learner with `plot_prediction()`:
...
rf_learner = lrn(...)
plot_prediction(rf_learner, spirals_task)
```
