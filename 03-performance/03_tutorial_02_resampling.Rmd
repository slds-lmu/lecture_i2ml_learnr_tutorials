...
- Get to know how to apply `benchmark` experiments
- Interpretation and visualization of the benchmark result
...
For this exercise we want to evaluate the learner with a 10-fold cross validation. Define the resampling using the function `makeResampleDesc()`:
...
```{r resample-hint-2}
# For the task use the build in `iris.task` or define it by yourself. Note, if you define the task by yourself use `id = "iris.example"`
task = iris.task
task = makeClassifTask(id = "iris-example", data = iris, target = "Species")
```
...
**Note:** The performance measure can also be specified. But in this example we are using the default measures which is, for multiclass classification, the mean misclassification error.
