...
You can use the function `rpart.plot()` from the package of the same name `rpart.plot` to visualize the tree structure. For this, you have to access the internal fitted model which is stored as `state$model` element of the fitted model. Additionally, call `plot_prediction()` to visualize the decision boundaries:
...
rpart.plot(learner_cart$state$model)
```

```{r cart-viz-hint-3}
# Call 'plotLearnerPrediction()' on the learner and task to get decision boundaries
plot_prediction(learner = learner_cart, task = spirals_task)
```
