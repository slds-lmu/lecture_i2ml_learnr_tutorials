...
- See how feature importance can be calculated with the random forest

*Practical (P)*

- Know how to train a random forest model using `mlr`
- Effect of the number of trees on the performance
...
Now its time to try different values for the number of trees and see if this has any influence on the performance. We also want to compare the random forests with a single CART. Therefore, define four different learners:
...
After defining the learner conduct the benchmark using the `benchmark()` function. The interesting measures are the `auc` and `mmce`. Use a 10-fold cross-validation as resampling technique. Finally, visualize the benchmark with `plotBMRBoxplots()`.
...
question("Which statements are true?",
  ...
  answer("Trying different values for the number of trees does not have an effect on the performance.", correct = TRUE),
